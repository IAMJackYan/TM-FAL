...
parser.add_argument('--model_pool_step', type=int, default=2, help='cosine')
parser.add_argument('--model_pool_size', type=int, default=20, help='cosine')

parser.add_argument('--s_step', type=int, default=1, help='cosine')

parser.add_argument('--cluster_size', type=int, default=10, help='cluster')
...
if __name__ == '__main__':
    ...
    assert al_method in ['Random', 'FEAL', 'ours', 'ours2']
    ...
    for al_round_idx in tqdm(range(al_round), ncols=100):
        ...
        for client_idx in range(client_num):
            ...
            if dataset in ['FedISIC']:
                ...
                optimizer = torch.optim.Adam(local_models[client_idx].parameters(), lr=args.base_lr, weight_decay=1e-5)
                scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[50], gamma=0.1)
            ...
            if round_idx == max_round-1 and al_round_idx < al_round-1:
                ...
                for client_idx in range(client_num):
                    ...
                    if len(local_unlabeled_sets[client_idx]) <= query_num[client_idx]:
                        ...
                        if query_model == 'both':
                            target_model_pools=model_pools[:args.model_pool_size]+local_model_pools[client_idx][:args.model_pool_size]

...
def prepare_isic():
    img_path = glob('/home/ylyan/data/ISIC_2019_Training_Input_preprocessed/*')
    for i in range(len(img_path)):
        # pdb.set_trace()
        img = Image.open(img_path[i])
        img_np = np.asarray(img)
        np.save('/home/ylyan/data/FedISIC_npy/{}'.format(img_path[i].split('/')[-1].replace('jpg','npy')), img_np)


def prepare_camelyon():
    data_dict = np.load('/home/ylyan/data/data.pkl', allow_pickle=True)
